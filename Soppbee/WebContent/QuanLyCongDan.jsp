...
Login users = null ;
 ...
 if(session.getAttribute("ipUserName") != null) {
	...
	int firstResult = 0, maxResult = 0,MaNM = 0;
       
       NguoiMua nguoimua = new NguoiMua();
     DuAn duan = new DuAn();
     SearchCDDAO search = new SearchCDDAO();
   		//List<NguoiMua> listNguoimua = show.getListNguoimua( MaNM, firstResult, maxResult);
%>
...
<script type="text/javascript">js/confirm.js</script>
...
<body class="page-homepage-carousel">
            ...
            <h2>Hồ sơ người mua nhà</h2>
			       <div class="card-header card-header-icon" data-background-color="rose">
                              
								 <br>
								   <div>
										<div style = "float: left">
											 <a href ="ThemCongDan.jsp"><button type="button" rel="tooltip" class="btn btn-success" style="border-radius: 8px;">
                                                           Thêm hồ sơ
                                                        </button></a>
										</div>
				...
				<form action="SearchCD" method = "post">
                            ...
                            for (NguoiMua list : search.searchTen()) {
                            	
                            	%>	 	
												 <input type ="hidden" name = "timkiem" value ="timkiem"/>
												<input type="submit" rel="tooltip" class="btn btn-success" style="border-radius: 8px; float: right;" value ="Tìm kiếm" />
            ...
            <div class="col-md-12">
                                        ...
                                        <table class="table table-striped table-bordered table-hover" id="myTable">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Tên dự án</th>
                                                    <th>Số điện thoại</th>
                                                    <th class="text-right">Tên phòng</th>
                            ...
                            for (NguoiMua list : show.getListNguoimua()) {
                        ...
                        %>
                                                <tr>                                               
                                                    ...
                                                    <td><%= list.getCMND() %></td>                       				
                                  					<td> <%= list.getTenDA() %></td>
                                  					<td><%= list.getSDT() %></td>
                                                    <td class="text-right"><%= list.getTenPhong() %></td>
                                                    <td class="text-right"><%= list.getDiaChi() %></td>
                                                    <td class="td-actions text-right">
                                                     
                                                   <%if (users.getIDR() == 2 || users.getIDR() == 3){ %> 
                                                    
                                                       <a href="Show_editCD?idcd=<%=list.getCMND()%>"> <button type="button"   rel="tooltip" class="btn btn-info" style="border-radius: 8px;">Sửa                                                      
                                                        </button></a> 
                                                       <a  class = "file"  href = "<%= deleteURL%>" > <button type="button" rel="tooltip" class="btn btn-danger remove" style="border-radius: 8px;"   onclick="return confirm('bạn có chắc xóa?')"  >
                                                           Xóa
                                                       </button> </a> 
